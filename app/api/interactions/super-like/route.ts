...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await InteractionService.superLike(authUser.userId, body.targetId, body.context)
    return NextResponse.json({ success: true, data: result })
  } catch (error) {
    console.error('[interaction.superLike] failed', error)
    const message = error instanceof Error ? error.message : 'Unable to super like member'
